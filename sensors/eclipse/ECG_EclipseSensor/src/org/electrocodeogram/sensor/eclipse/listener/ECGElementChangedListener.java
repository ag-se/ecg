								...
								for(int j=0 ;j < methods.length ; j++){
									...
									if(!methodtable.containsKey(methodhandle))
										...
										sensor.processActivity("msdt.exactcodechange.xsd",
				    						...
				    						+ "<elementName>"+methods[j].getElementName()+"</elementName>"
				    						+ "<identifier><![CDATA["+methodhandle+ "]]></identifier>"
				    						+ "<codeOrIdentifier><![CDATA["+methods[j].getSource()+ "]]>"
		        	...
		        	for(int j=0 ;j < methods.length ; j++){
		        		...
		        		if(!oldmethod.equals(methods[j].getSource())){
		        			...
		        			sensor.processActivity("msdt.exactcodechange.xsd",
		        					...
		        					+ "<elementName>"+methods[j].getElementName()+"</elementName>"
		        					+ "<identifier><![CDATA["+methodhandle+ "]]>"
		        					+ "</identifier><codeOrIdentifier><![CDATA["+methods[j].getSource()+ "]]>"
		        					+ "</codeOrIdentifier></change>"
		        	                + "</exactCodeChange></microActivity>");
		        		}
		        	}
					...
					if(elementtype == IJavaElement.METHOD){
		    		 	...
		    		 	if(children_2[i].getKind() == IJavaElementDelta.ADDED){ // if method was added
		    		 		...
		    		 		ecg_event = "<?xml version=\"1.0\"?><microActivity><commonData><username>"
		    					...
		    					+ "<change><typeOfChange>ADDED</typeOfChange>"
		    					+ "<elementName>"+method.getElementName()+"</elementName>"
		    					+ "<identifier><![CDATA["+method.getHandleIdentifier()+ "]]>"
		    					+ "</identifier><codeOrIdentifier><![CDATA["+method.getSource()+ "]]>"
		    					+ "</codeOrIdentifier></change>"
		                        + "</exactCodeChange></microActivity>";
		    		 		// add to methods
					    	...
					    	while(keys.hasMoreElements()){
					    		...
					    		if(((String)methodtable.get(identifier)).endsWith(sub)){
					    			...
					    			ecg_event = "<?xml version=\"1.0\"?><microActivity><commonData><username>"
				    					...
				    					+ "<elementName>"+method.getElementName()+"</elementName>"
				    					+ "<identifier><![CDATA["+method.getHandleIdentifier()+ "]]>"
				    					+ "</identifier><codeOrIdentifier><![CDATA["+identifier+ "]]>"
				    					+ "</codeOrIdentifier></change>"
				                        + "</exactCodeChange></microActivity>";
					    			...
					    			ecg_event += "<?xml version=\"1.0\"?><microActivity><commonData><username>"
				                        ...
				                        + method.getAncestor(IType.TYPE).getElementName()+"</path>"
				    					+ "<change><typeOfChange>CHANGED</typeOfChange>"
				    					+ "<elementName>"+new_elementname+"</elementName>"
				    					+ "<identifier><![CDATA["+identifier+ "]]>"
				    					+ "</identifier><codeOrIdentifier><![CDATA["+(String)methodtable.get(identifier)+ "]]>"
				    					+ "</codeOrIdentifier></change>"
				                        + "</exactCodeChange></microActivity>";
