...
public class RefreshViewsJob extends Job {
	...
	protected IStatus run(IProgressMonitor aMonitor) {
		try {
//			PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().findView(VisitedElementsView.ID);
//			VisitedElementsView visitedElementsView = (VisitedElementsView)PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().findView(VisitedElementsView.ID);
