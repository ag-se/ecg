...
/*
 * vw:
 * this is version diff_match_patch_20070706.zip from http://code.google.com/p/google-diff-match-patch/
 * downloaded on 2007-09-09.
 * 
 * Modifications:
 * - The class was renamed from diff_match_patch to DiffMatchPatch.
 * - The internal classes Diff and Patch were moved to separate files.
 * - They were renamed to DMPDiff and DMPPatch
 */
...
public class DiffMatchPatch
{
	 ...
	 */
	public LinkedList<DMPDiff> diff_main(String text1, String text2)
	{
	 ...
	 * @return Linked List of Diff objects
	 */
	public LinkedList<DMPDiff> diff_main(String text1, String text2, boolean checklines)
	{
		// Check for equality (speedup)
		LinkedList<DMPDiff> diffs;
		if (text1.equals(text2))
		{
			diffs = new LinkedList<DMPDiff>();
			diffs.add(new DMPDiff(Operation.EQUAL, text1));
			return diffs;
		}
		...
		{
			diffs.addFirst(new DMPDiff(Operation.EQUAL, commonprefix));
		}
		if (commonsuffix.length() != 0)
		{
			diffs.addLast(new DMPDiff(Operation.EQUAL, commonsuffix));
		}
	...
	public LinkedList<DMPDiff> diff_compute(String text1, String text2, boolean checklines)
	{
		LinkedList<DMPDiff> diffs = new LinkedList<DMPDiff>();

		if (text1.length() == 0)
		{
			// Just add some text (speedup)
			diffs.add(new DMPDiff(Operation.INSERT, text2));
			return diffs;
		}
		...
		if (text2.length() == 0)
		{
			...
			diffs.add(new DMPDiff(Operation.DELETE, text1));
			return diffs;
		}
		...
		if (i != -1)
		{
			...
			diffs.add(new DMPDiff(op, longtext.substring(0, i)));
			diffs.add(new DMPDiff(Operation.EQUAL, shorttext));
			diffs.add(new DMPDiff(op, longtext.substring(i + shorttext.length())));
			return diffs;
		}
		...
		if (hm != null)
		{
			...
			LinkedList<DMPDiff> diffs_a = diff_main(text1_a, text2_a, checklines);
			LinkedList<DMPDiff> diffs_b = diff_main(text1_b, text2_b, checklines);
			// Merge the results.
			diffs = diffs_a;
			diffs.add(new DMPDiff(Operation.EQUAL, mid_common));
		...
		if (checklines)
		{
			...
			diffs.add(new DMPDiff(Operation.EQUAL, ""));
			int count_delete = 0;
			int count_insert = 0;
			String text_delete = "";
			String text_insert = "";
			ListIterator<DMPDiff> pointer = diffs.listIterator();
			DMPDiff thisDiff = pointer.next();
			while (thisDiff != null)
			{
				...
				else
				{
					...
					if (count_delete >= 1 && count_insert >= 1)
					{
						...
						for (DMPDiff newDiff : diff_main(text_delete, text_insert, false))
						{
							pointer.add(newDiff);
						}
					}
	 ...
	 * @param linearray List of unique strings
	 */
	protected void diff_charsToLines(LinkedList<DMPDiff> diffs, List<String> linearray)
	{
		String text;
		for (DMPDiff diff : diffs)
		{
			...
			{
				text += linearray.get(diff.text.charAt(y));
			}
			diff.text = text;
		}
	}
	 ...
	 */
	protected LinkedList<DMPDiff> diff_map(String text1, String text2)
	{
		...
		for (int d = 0; d < max_d; d++)
		{
			...
			for (int k = -d; k <= d; k += 2)
			{
				...
				else if (done)
				{
					...
					LinkedList<DMPDiff> a = diff_path1(v_map1, text1.substring(0, x), text2.substring(0, y));
			...
			if (doubleEnd)
			{
				...
				for (int k = -d; k <= d; k += 2)
				{
					...
					if (done)
					{
						...
						LinkedList<DMPDiff> a = diff_path1(v_map1, text1.substring(0, text1.length() - x), text2
								.substring(0, text2.length() - y));
						a.addAll(diff_path2(v_map2, text1.substring(text1.length() - x), text2.substring(text2.length()
								- y)));
						return a;
					}
				}
	 ...
	 */
	protected LinkedList<DMPDiff> diff_path1(List<Set<String>> v_map, String text1, String text2)
	{
		LinkedList<DMPDiff> path = new LinkedList<DMPDiff>();
		int x = text1.length();
		int y = text2.length();
		...
		for (int d = v_map.size() - 2; d >= 0; d--)
		{
			while (true)
			{
				if (v_map.get(d).contains((x - 1) + "," + y))
				{
					...
					{
						path.addFirst(new DMPDiff(Operation.DELETE, text1.substring(x, x + 1)));
					}
				...
				else if (v_map.get(d).contains(x + "," + (y - 1)))
				{
					...
					{
						path.addFirst(new DMPDiff(Operation.INSERT, text2.substring(y, y + 1)));
					}
				...
				else
				{
					...
					{
						path.addFirst(new DMPDiff(Operation.EQUAL, text1.substring(x, x + 1)));
					}
	 ...
	 * @return LinkedList of Diff objects
	 */
	protected LinkedList<DMPDiff> diff_path2(List<Set<String>> v_map, String text1, String text2)
	{
		LinkedList<DMPDiff> path = new LinkedList<DMPDiff>();
		int x = text1.length();
		int y = text2.length();
		...
		for (int d = v_map.size() - 2; d >= 0; d--)
		{
			while (true)
			{
				if (v_map.get(d).contains((x - 1) + "," + y))
				{
					...
					{
						path.addLast(new DMPDiff(Operation.DELETE, text1.substring(text1.length() - x - 1, text1
								.length()
								- x)));
					}
					last_op = Operation.DELETE;
				...
				else if (v_map.get(d).contains(x + "," + (y - 1)))
				{
					...
					{
						path.addLast(new DMPDiff(Operation.INSERT, text2.substring(text2.length() - y - 1, text2
								.length()
								- y)));
					}
					last_op = Operation.INSERT;
				...
				else
				{
					...
					{
						path.addLast(new DMPDiff(Operation.EQUAL, text1.substring(text1.length() - x - 1, text1
								.length()
								- x)));
					}
					last_op = Operation.EQUAL;
	...
	public int diff_commonPrefix(String text1, String text2)
	{
		...
		{
			if (text1.regionMatches(pointerstart, text2, pointerstart, pointermid - pointerstart))
			{
	...
	public int diff_commonSuffix(String text1, String text2)
	{
		...
		while (pointermin < pointermid)
		{
			if (text1.regionMatches(text1.length() - pointermid, text2, text2.length() - pointermid, pointermid
					- pointerend))
			{
				pointermin = pointermid;
	...
	protected String[] diff_halfMatch(String text1, String text2)
	{
		...
		String[] hm1 = diff_halfMatchI(longtext, shorttext, (int) Math.ceil(longtext.length() / 4));
		// Check again based on the third quarter.
		String[] hm2 = diff_halfMatchI(longtext, shorttext, (int) Math.ceil(longtext.length() / 2));
		String[] hm;
	 ...
	 * @param diffs LinkedList of Diff objects
	 */
	public void diff_cleanupSemantic(LinkedList<DMPDiff> diffs)
	{
		if (diffs.isEmpty())
		{
			return;
		...
		Stack<DMPDiff> equalities = new Stack<DMPDiff>(); // Stack of qualities.
		...
		while (thisDiff != null)
		{
			...
			else
			{
						...
						&& (lastequality.length() <= length_changes2))
				{
					...
					pointer.set(new DMPDiff(Operation.DELETE, lastequality));
					// Insert a coresponding an insert.
					pointer.add(new DMPDiff(Operation.INSERT, lastequality));
	 ...
	 * @param diffs LinkedList of Diff objects
	 */
	public void diff_cleanupSemanticLossless(LinkedList<DMPDiff> diffs)
	{
		...
		ListIterator<DMPDiff> pointer = diffs.listIterator();
		DMPDiff prevDiff = pointer.hasNext() ? pointer.next() : null;
		DMPDiff thisDiff = pointer.hasNext() ? pointer.next() : null;
		DMPDiff nextDiff = pointer.hasNext() ? pointer.next() : null;
	...
	private int diff_cleanupSemanticScore(String one, String two, String three)
	{
		...
		if (Character.isWhitespace(two.charAt(two.length() - 1)) || Character.isWhitespace(three.charAt(0)))
		{
			score++;
		}
		return score;
	 ...
	 * @param diffs LinkedList of Diff objects
	 */
	public void diff_cleanupEfficiency(LinkedList<DMPDiff> diffs)
	{
		if (diffs.isEmpty())
		{
			return;
		...
		boolean changes = false;
		Stack<DMPDiff> equalities = new Stack<DMPDiff>(); // Stack of equalities.
		String lastequality = null; // Always equal to equalities.lastElement().text
		ListIterator<DMPDiff> pointer = diffs.listIterator();
		...
		DMPDiff thisDiff = pointer.next();
		DMPDiff safeDiff = thisDiff; // The last Diff that is known to be unsplitable.
		while (thisDiff != null)
		{
			if (thisDiff.operation == Operation.EQUAL)
			{
			...
			else
			{
								...
								+ (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1 : 0)) == 3)))
				{
					...
					pointer.set(new DMPDiff(Operation.DELETE, lastequality));
					// Insert a coresponding an insert.
					pointer.add(thisDiff = new DMPDiff(Operation.INSERT, lastequality));
	 ...
	 * @param diffs LinkedList of Diff objects
	 */
	public void diff_cleanupMerge(LinkedList<DMPDiff> diffs)
	{
		diffs.add(new DMPDiff(Operation.EQUAL, "")); // Add a dummy entry at the end.
		ListIterator<DMPDiff> pointer = diffs.listIterator();
		int count_delete = 0;
		int count_insert = 0;
		String text_delete = "";
				...
				if (count_delete != 0 || count_insert != 0)
				{
					...
					if (count_delete != 0 && count_insert != 0)
					{
						...
						if (commonlength != 0)
						{
							...
							{
								pointer.add(new DMPDiff(Operation.EQUAL, text_insert.substring(0, commonlength)));
							}
					...
					{
						pointer.add(new DMPDiff(Operation.DELETE, text_delete));
					}
					if (text_insert.length() != 0)
					{
						pointer.add(new DMPDiff(Operation.INSERT, text_insert));
					}
	 ...
	 * @param diffs LinkedList of Diff objects
	 */
	public void diff_addIndex(LinkedList<DMPDiff> diffs)
	{
		int i = 0;
		for (DMPDiff aDiff : diffs)
		{
			aDiff.index = i;
	 ...
	 */
	public int diff_xIndex(LinkedList<DMPDiff> diffs, int loc)
	{
		int chars1 = 0;
		int chars2 = 0;
		int last_chars1 = 0;
		int last_chars2 = 0;
		DMPDiff lastDiff = null;
		for (DMPDiff aDiff : diffs)
		{
	 ...
	 */
	public String diff_prettyHtml(LinkedList<DMPDiff> diffs)
	{
		diff_addIndex(diffs);
		String html = "";
		String text;
		for (DMPDiff aDiff : diffs)
		{
			...
			if (aDiff.operation == Operation.DELETE)
			{
				html += "<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i=" + aDiff.index + "\">" + text + "</DEL>";
			}
			else if (aDiff.operation == Operation.INSERT)
			{
				html += "<INS STYLE=\"background:#E6FFE6;\" TITLE=\"i=" + aDiff.index + "\">" + text + "</INS>";
			}
			else
			{
				html += "<SPAN TITLE=\"i=" + aDiff.index + "\">" + text + "</SPAN>";
			}
		}
		return html;
	}
	...
	public int match_bitap(String text, String pattern, int loc)
	{
		...
		if (best_loc != -1)
		{
			score_threshold = Math.min(match_bitapScore(0, best_loc, loc, score_text_length, pattern), score_threshold);
		}
		...
		for (int d = 0; d < pattern.length(); d++)
		{
			...
			for (int j = finish - 1; j >= start; j--)
			{
				...
				else
				{
					...
					rd[j] = ((rd[j + 1] << 1) | 1) & (s.containsKey(text.charAt(j)) ? s.get(text.charAt(j)) : 0)
							| ((last_rd[j + 1] << 1) | 1) | ((last_rd[j] << 1) | 1) | last_rd[j + 1];
				}
				if ((rd[j] & matchmask) != 0)
				{
	 ...
	 */
	public void patch_addContext(DMPPatch patch, String text)
	{
		...
		if (prefix.length() != 0)
		{
			patch.diffs.addFirst(new DMPDiff(Operation.EQUAL, prefix));
		}
		// Add the suffix.
		...
		if (suffix.length() != 0)
		{
			patch.diffs.addLast(new DMPDiff(Operation.EQUAL, suffix));
		}
	 ...
	 */
	public LinkedList<DMPPatch> patch_make(String text1, String text2)
	{
		// No diffs provided, compute our own.
		LinkedList<DMPDiff> diffs = diff_main(text1, text2, true);
		if (diffs.size() > 2)
		{
			diff_cleanupSemantic(diffs);
	 ...
	 * @return LinkedList of Patch objects.
	 */
	public LinkedList<DMPPatch> patch_make(String text1, String text2, LinkedList<DMPDiff> diffs)
	{
		LinkedList<DMPPatch> patches = new LinkedList<DMPPatch>();
		if (diffs.isEmpty())
		{
			return patches; // Get rid of the null case.
		}
		DMPPatch patch = new DMPPatch();
		...
		for (DMPDiff aDiff : diffs)
		{
			...
			else if (aDiff.operation == Operation.EQUAL && aDiff.text.length() <= 2 * Patch_Margin
			...
			if (aDiff.operation == Operation.EQUAL && aDiff.text.length() >= 2 * Patch_Margin)
			{
	 ...
	 *      boolean values
	 */
	public Object[] patch_apply(LinkedList<DMPPatch> patches, String text)
	{
		...
		for (DMPPatch aPatch : patches)
		{
			...
			else
			{
				...
				if (text1.equals(text2))
				{
					...
					text = text.substring(0, start_loc) + aPatch.text2() + text.substring(start_loc + text1.length());
				}
				else
				{
					...
					diff_cleanupSemanticLossless(diffs);
					index1 = 0;
					for (DMPDiff aDiff : aPatch.diffs)
					{
	 ...
	 */
	public void patch_splitMax(LinkedList<DMPPatch> patches)
	{
		int patch_size;
		...
		Operation diff_type;
		String diff_text;
		ListIterator<DMPPatch> pointer = patches.listIterator();
		DMPPatch bigpatch = pointer.hasNext() ? pointer.next() : null;
		while (bigpatch != null)
		{
			...
			while (!bigpatch.diffs.isEmpty())
			{
				...
				if (precontext.length() != 0)
				{
					...
					patch.diffs.add(new DMPDiff(Operation.EQUAL, precontext));
				}
				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin)
				{
					...
					else
					{
						...
						patch.diffs.add(new DMPDiff(diff_type, diff_text));
				...
				if (postcontext.length() != 0)
				{
					...
					{
						patch.diffs.add(new DMPDiff(Operation.EQUAL, postcontext));
					}
				}
				if (!empty)
				{
	 ...
	 */
	public String patch_toText(List<DMPPatch> patches)
	{
		String text = "";
		for (DMPPatch aPatch : patches)
		{
	 ...
	 */
	public List<DMPPatch> patch_fromText(String textline)
	{
		List<DMPPatch> patches = new LinkedList<DMPPatch>();
		...
		while (!text.isEmpty())
		{
			...
			m.matches();
			assert m.groupCount() == 4 : "Invalid patch string:\n" + text.getFirst();
			patch = new DMPPatch();
			patches.add(patch);
			...
			while (!text.isEmpty())
			{
				...
				{
					// Deletion.
					patch.diffs.add(new DMPDiff(Operation.DELETE, line));
				}
				else if (sign == '+')
				{
					// Insertion.
					patch.diffs.add(new DMPDiff(Operation.INSERT, line));
				}
				else if (sign == ' ')
				{
					// Minor equality.
					patch.diffs.add(new DMPDiff(Operation.EQUAL, line));
				}
				else if (sign == '@')
				{
					// Start of next patch.
