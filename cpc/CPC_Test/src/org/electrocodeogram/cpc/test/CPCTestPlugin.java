package org.electrocodeogram.cpc.test;
 ...
 */
public class CPCTestPlugin extends Plugin
{
	private static Log log = LogFactory.getLog(CPCTestPlugin.class);

	// The plug-in ID
	public static final String PLUGIN_ID = "org.electrocodeogram.cpc.test";

	/**
	 * Number of milliseconds to wait after a document modification before
	 * the clone positions can be verified.
	 * This is needed as some changes are processed asynchronously.
	 * On slow systems it may be required to increase this value to get some
	 * of the JUnit test to succeed.
	 */
	public static final int TEST_PRECHECK_DELAY = 500;

	// The shared instance
	private static CPCTestPlugin plugin;
	 ...
	 */
	public static CPCTestPlugin getDefault()
	{
		return plugin;
	}
	...
	//		log = LogFactory.getLog(CPCTestPlugin.class);
	//		log.info("test");

	// for now we use a shared logger.
	//		logManager = ECGEclipseCorePlugin.getDefault().getPluginLogManager();
	//		log = this.logManager.getLogger(CPCTestPlugin.class.getName());
	//		log.info("using shared logging");
