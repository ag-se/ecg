...
{
	private static final Log log = LogFactory.getLog(ReplayView.class);

	public static final String VIEW_ID = "org.electrocodeogram.cpc.ui.views.replayview"; //$NON-NLS-1$
	...
	/*
	 * Keep track of listeners, we'll have to unregister them on dispose.
	 */
	private IReplayActionListener beginButtonReplayActionListener;
	private IReplayActionListener timeToNextStepLabelReplayActionListener;
	private IModelChangeListener timeToNextStepLabelModelChangeListener;
	private IModelChangeListener timestamplabelModelChangeListener;
	private IModelChangeListener changeTypeLabelModelChangeListener;
	...
	{
		/*
		 * Remove ReplayView specific listeners.
		 */
		ReplayControl replayControl = ReplayControl.getInstance();
		replayControl.removeReplayActionListener(beginButtonReplayActionListener);
		replayControl.removeReplayActionListener(timeToNextStepLabelReplayActionListener);

		DataProvider dataProvider = DataProvider.getInstance();
		dataProvider.removeModelChangeListener(timeToNextStepLabelModelChangeListener);
		dataProvider.removeModelChangeListener(timestamplabelModelChangeListener);
		dataProvider.removeModelChangeListener(changeTypeLabelModelChangeListener);

		DataProvider.getInstance().removeModelChangeListener(this);
		if (replayview != null)
			replayview.dispose();
	...
	public void createPartControl(Composite parent)
	{
		if (log.isTraceEnabled())
			log.trace("createPartControl() - parent: " + parent);
		...
		// -----------------------------------
		Composite buttons = new Composite(group2, SWT.NONE);
		Composite labels = new Composite(group2, SWT.NONE);
		gridLayout = new GridLayout();
		...
		beginButtonReplayActionListener = new IReplayActionListener()
		{
		...
		ReplayControl.getInstance().addReplayActionListener(beginButtonReplayActionListener);
		...
		speedBurstModeText.setFont(new Font(parent.getDisplay(), new FontData(
				parent.getFont().getFontData().toString(), 10, SWT.NORMAL)));
		...
		timeToNextStep.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData().toString(),
				10, SWT.NORMAL)));
		...
		timeToNextStepLabel.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData()
				.toString(), 12, SWT.NORMAL)));
		...
		timeToNextStepLabelReplayActionListener = new IReplayActionListener()
		{
			public void ReplayAction(ReplayActionEvent event)
			{
				if (event.getCause() == ReplayActionEvent.COUNTDOWN_CHANGED
						&& ReplayControl.getInstance().isTimerRunning())
				{
		...
		ReplayControl.getInstance().addReplayActionListener(timeToNextStepLabelReplayActionListener);

		timeToNextStepLabelModelChangeListener = new IModelChangeListener()
		{
			public void modelChange(ModelChangeEvent event)
			{
				if (event.getCause() == ModelChangeEvent.REPLAY_CHANGED
						|| event.getCause() == ModelChangeEvent.ELEMENT_CHANGED)
					timeToNextStepLabel.setText(new SimpleDateFormat("mm:ss").format(new Date(0)));
			}
		};
		DataProvider.getInstance().addModelChangeListener(timeToNextStepLabelModelChangeListener);

		// ------------------
		// textlabel
		// ------------------
		Label timeStampText = new Label(labels, SWT.HORIZONTAL | SWT.CENTER);
		timeStampText.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData().toString(), 10,
				SWT.NORMAL)));
		...
		timestamplabel.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData().toString(),
				12, SWT.NORMAL)));
		...
		timestamplabelModelChangeListener = new IModelChangeListener()
		{
			public void modelChange(ModelChangeEvent event)
			{
				if (event.getCause() == ModelChangeEvent.ELEMENT_CHANGED)
				{
					String timestamp = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss").format(DataProvider.getInstance()
							.getActiveReplay().getCurrentElement().getTimestamp());
					timestamplabel.setText(" " + timestamp + " ");
		...
		DataProvider.getInstance().addModelChangeListener(timestamplabelModelChangeListener);

		//		 ------------------
		// textlabel
		// ------------------
		Label changeTypeText = new Label(labels, SWT.HORIZONTAL | SWT.CENTER);
		changeTypeText.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData().toString(),
				10, SWT.NORMAL)));
		changeTypeText.setText(" Changed by : ");

		// ---------------------
		// Change type label
		// ---------------------
		final Label changeTypeLabel = new Label(labels, SWT.HORIZONTAL | SWT.CENTER | SWT.BORDER);
		changeTypeLabel.setFont(new Font(parent.getDisplay(), new FontData(parent.getFont().getFontData().toString(),
				12, SWT.NORMAL)));
		changeTypeLabel.setText("  < Creator >  ");
		...
		changeTypeLabelModelChangeListener = new IModelChangeListener()
		{
			public void modelChange(ModelChangeEvent event)
			{
				if (event.getCause() == ModelChangeEvent.ELEMENT_CHANGED)
		...
		DataProvider.getInstance().addModelChangeListener(changeTypeLabelModelChangeListener);

		log.trace("createPartControl() - done.");
	...
	{
		if (log.isTraceEnabled())
			log.trace("modelChange() - event: " + event);
	...
	private class ReplayEditor implements IModelChangeListener
	{
		...
		{
			if (log.isTraceEnabled())
				log.trace("ReplayEditor() - group: " + group);
		...
		{
			if (log.isTraceEnabled())
				log.trace("ReplayEditor.setFontSize() - size: " + size);
		...
		public void setFocus()
		{
			log.trace("ReplayEditor.setFocus()");
		...
		{
			if (log.isTraceEnabled())
				log.trace("ReplayEditor.modelChange() - event: " + event);
			...
			if (event.getCause() == ModelChangeEvent.ELEMENT_CHANGED)
			{
				...
				for (int i = 0; i < styleranges.length; i++)
				{
					if (styleranges[i].length > 0)
						...
						textviewer.setLineBackground(textviewer.getLineAtOffset(styleranges[i].start), 1,
								styleranges[i].background);
				}

				int offsetToReveal = styleranges[styleranges.length - 1].start
						+ styleranges[styleranges.length - 1].length;
				if (isLineDelimiter(offsetToReveal))
					//we're in the middle of a \r\n line break, we can't put the selection there
					//lets put it in front of the line break.
					--offsetToReveal;

				textviewer.setSelection(offsetToReveal);
			}
		}

		/**
		 * Copied from: org.eclipse.swt.custom.StyledText.isLineDelimiter(int)
		 * 
		...
		{
			StyledTextContent content = textviewer.getContent();
