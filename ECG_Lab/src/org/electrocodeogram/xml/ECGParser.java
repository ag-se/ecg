...
public class ECGParser
{
	...
	private static Logger _logger = LogHelper.createLogger(ECGParser.class.getName());
	 ...
	 */
	private static ModuleType getModuleType(Document document) throws PropertyException, ModuleSetupLoadException
	{
	...
	private static ModuleProperty[] getModuleProperties(Document document) throws PropertyException
	{
		...
		for (Node propertyNode : propertyNodes)
		{
			...
			catch (ClassNotFoundException e)
			{
				...
				{
					_logger.log(Level.WARNING,"Error while reading the property type: " + modulePropertyType);
			...
			if(type == null)
			{
				...
				throw new PropertyException("The property type is not a known Class: " + modulePropertyType);
			}
	...
	private static Node getChildNode(Node parentNode, String nodeName) throws PropertyException
	{
		...
		{
			throw new PropertyException("The node " + parentNode.getNodeName() + " does not have a child with name: " + nodeName);
		}
