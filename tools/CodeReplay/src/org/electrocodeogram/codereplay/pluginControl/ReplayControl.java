...
import org.electrocodeogram.codereplay.eventIO.EventReader;
import org.electrocodeogram.codereplay.eventIO.EventWriter;
...
public class ReplayControl implements IModelChangeListener{
	...
	// ----------------------------------
	// IMPORTANT: enter new readers here!!!!!
	// ----------------------------------
	private AbstractReader[] readers = {EventReader.getInstance()};
//	 ----------------------------------
	// IMPORTANT: enter new writers here!!!!!
	// ----------------------------------
	private AbstractWriter[] writers = {EventWriter.getInstance()};
	
	private AbstractReader activeReader;

	private AbstractWriter activeWriter;
	...
	/**
	 * @return an array of all available readers
	 */
	public AbstractReader[] getReaders(){
		return readers;
	}
	/**
	 * @return an array of all available writers
	 */
	public AbstractWriter[] getWriters(){
		return writers;
	}
	
	/**
	 * IMPORTANT: The number of the reader is determined by its position in the array
	 * you get by calling {@link ReplayControl.getReaders}
	 * 
	 * @param no number of reader to be set as active reader
	 */
	public void setActiveReader(int no){
		activeReader = readers[no];		
	}
	/**
	 * IMPORTANT: The number of the writer is determined by its position in the array
	 * you get by calling {@link ReplayControl.getWriters}
	 * 
	 * @param no number of writer to be set as active writer
	 */
	public void setActiveWriter(int no){
		activeWriter = writers[no];
	}
	...
	/**
	 * Opens the specified file(by calling openFile() on the active reader).
	 * 
	...
	public void openFile(File file) {
		...
		try{
			activeReader.openFile(file);
		}catch(Exception e){e.printStackTrace();}
	}
	
	/**
	 * Saves all data to the specified file(by calling write() on the active writer).
	 * 
	 * @param file the file to save to
	 */
	public void saveFile(File file) {
		stopReplay();
		Enumeration replays = DataProvider.getInstance().getReplays();
		try {
			activeWriter.write(file, replays);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
