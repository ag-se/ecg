...
public abstract class Module extends Observable implements Observer
{
    ...
    private void loadMDTDSchemas() throws FileNotFoundException
    {
        ...
        File msdtDir = new File(moduleDirString + File.separator + msdtSubDirString);
        ...
        if (defs != null) {
            for (int i = 0; i < defs.length; i++) {
                ...
                try {
                    ...
                    this.providedMsdt.add(microSensorDataType);
                    
                    this.logger.log(Level.INFO, "Module " + this.getName() + " loaded additional MicroSensorDatyType " + defFile.getName());
                    
                    Core.getInstance().getMsdtRegistry().registerMsdt(microSensorDataType);
